...
## Load in the files ----
all.clean.split <- read_csv(paste0("../data/text_split/",story.meta,".clean.split.csv"),
                            show_col_type = F)

text1.clean.split <- all.clean.split |> filter(story == 1)
text2.clean.split <- all.clean.split |> filter(story == 2)
text3.clean.split <- all.clean.split |> filter(story == 3)
text4.clean.split <- all.clean.split |> filter(story == 4)

## Read in M-BERT embeddings ----

text1.embeds <- read_csv(paste0("../data/embeddings/mbert/",story.meta,"_s1_embeds_raw.csv"), show_col_type = F) |> 
  clean_names() |> 
  mutate(sent_num = 1:n()) 
text2.embeds <- read_csv(paste0("../data/embeddings/mbert/",story.meta,"_s2_embeds_raw.csv"), show_col_type = F)|> 
  clean_names() |> 
  mutate(sent_num = 1:n()) 
text3.embeds <- read_csv(paste0("../data/embeddings/mbert/",story.meta,"_s3_embeds_raw.csv"), show_col_type = F)|> 
  clean_names() |> 
  mutate(sent_num = 1:n()) 
text4.embeds <- read_csv(paste0("../data/embeddings/mbert/",story.meta,"_s4_embeds_raw.csv"), show_col_type = F)|> 
  clean_names() |> 
  mutate(sent_num = 1:n()) 

# Join with sentences.
text1.clean.split <- text1.clean.split |> 
  mutate(sent_num = 1:n()) |> 
  left_join(text1.embeds, by = "sent_num")

text2.clean.split <- text2.clean.split |> 
  mutate(sent_num = 1:n()) |> 
  left_join(text2.embeds, by = "sent_num")

text3.clean.split <- text3.clean.split |> 
  mutate(sent_num = 1:n()) |> 
  left_join(text3.embeds, by = "sent_num")

text4.clean.split <- text4.clean.split |> 
  mutate(sent_num = 1:n()) |> 
  left_join(text4.embeds, by = "sent_num")

all.clean.split_wEmb <- rbind(text1.clean.split, 
                            text2.clean.split, 
                            text3.clean.split,
                            text4.clean.split)
...
embeds3 <- all.clean.split_wEmb |> 
  ...
  mutate(sent_num.prop = sent_num/n()) |>
  ungroup() |>
  select(story, lang, sentences, sent_num, sent_num.prop, everything())

umap.embeds <- read_csv(paste0("../data/embeddings/",story.meta,".umap_embeds.csv"), 
                        show_col_type = F) |> 
  select(-story) |> 
...
ggsave(paths.A,
       filename = here(paste0("../figures/",story.meta,".umap-smoothed_full.png")),
       units = "in",
       dpi = 300,
       width = 10, height = 7)
...
## text1, text2 ----
text1.text2 <- dtw(umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text1.text2.distance <- text1.text2$normalizedDistance

## text1, text3 ----
text1.text3 <- dtw(umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text1.text3.distance <- text1.text3$normalizedDistance

## text1, text4 ----
text1.text4 <- dtw(umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text1.text4.distance <- text1.text4$normalizedDistance

## text2, text1 ----

text2.text1 <- dtw(umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text2.text1.distance <- text2.text1$normalizedDistance

## text2, text3 ----

text2.text3 <- dtw(umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text2.text3.distance <- text2.text3$normalizedDistance

## text2, text4 ----

text2.text4 <- dtw(umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text2.text4.distance <- text2.text4$normalizedDistance

## text3, text1 ----
text3.text1 <- dtw(umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text3.text1.distance <- text3.text1$normalizedDistance

## text3, text2 ----
text3.text2 <- dtw(umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text3.text2.distance <- text3.text2$normalizedDistance

## text3, text4 ----
text3.text4 <- dtw(umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text3.text4.distance <- text3.text4$normalizedDistance

## text4, text2 ----
text4.text2 <- dtw(umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text4.text2.distance <- text4.text2$normalizedDistance

## text4, text3 ----
text4.text3 <- dtw(umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text4.text3.distance <- text4.text3$normalizedDistance

## text4, text1 ----
text4.text1 <- dtw(umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text4.text1.distance <- text4.text1$normalizedDistance

text1.text1 <- dtw(umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 1) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text1.text1.distance <- text1.text1$normalizedDistance

text2.text2 <- dtw(umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 2) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text2.text2.distance <- text2.text2$normalizedDistance

text3.text3 <- dtw(umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 3) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text3.text3.distance <- text3.text3$normalizedDistance

text4.text4 <- dtw(umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 umap.embeds |> filter(story == 4) |> 
                   select(umap.x, umap.y),
                 keep = TRUE)
text4.text4.distance <- text4.text4$normalizedDistance

## Final heatmap ----

dtw.df <- data.frame(story = c("english1", "russian2","french3", "english4"),
                     english1 = c(text1.text1.distance, text2.text1.distance, text3.text1.distance, text4.text1.distance),
                     russian2 = c(text1.text2.distance, text2.text2.distance, text3.text2.distance, text4.text2.distance),
                     french3 = c(text1.text3.distance, text2.text3.distance, text3.text3.distance, text4.text3.distance),
                     english4 = c(text1.text4.distance, text2.text4.distance, text3.text4.distance, text4.text4.distance)) |> 
  melt() |> 
  mutate(x.story = as.numeric(str_sub(story, start = -1)),
         y.story = as.numeric(str_sub(variable, start = -1)), 
         translation.steps = (x.story - y.story),
         pair = paste(story, variable)) |> 
  rename(dtw.distance = value)

write_csv(dtw.df, file = paste0("../data/processed_data/",story.meta,"_dtw_values.csv"))
...
dtw.plot <- ggplot(dtw.df, aes(x = factor(story,
  ...
  geom_label(aes(label = round(dtw.distance, digits = 3)), fill = "white")+
  ggtitle(story.meta)

dtw.plot

ggsave(dtw.plot,
       filename = here(paste0("../figures/",story.meta,".dtw-standard.png")),
       units = "in",
       dpi = 300,
       width = 10, height = 7)
...
dtw.df |> 
  ...
  ggplot(aes(x = abs(translation.steps), y = dtw.distance))+
...
dtw.df <- data.frame(story = c("french1", "russian2","russian3", "french4"),
                     french1 = c(text1.text1.distance, text2.text1.distance, text3.text1.distance, text4.text1.distance),
                     russian2 = c(text1.text2.distance, text2.text2.distance, text3.text2.distance, text4.text2.distance),
                     russian3 = c(text1.text3.distance, text2.text3.distance, text3.text3.distance, text4.text3.distance),
                     french4 = c(text1.text4.distance, text2.text4.distance, text3.text4.distance, text4.text4.distance)) |> 
  melt() |> 
  mutate(x.story = as.numeric(str_sub(story, start = -1)),
         y.story = as.numeric(str_sub(variable, start = -1)),
         translation.steps = (x.story - y.story),
...
dtw.plot <- ggplot(dtw.df, aes(x = factor(story,
                                          levels = c("french1", "russian2","russian3", "french4")), 
                               y = factor(variable,
                                          levels = c("french1", "russian2","russian3", "french4")), 
                               fill = dtw.distance))+
  geom_tile()+
...
# 
# lm.df <- data.frame(dtw.distance = c(text1.text2.distance,
#                                      text1.text3.distance,
#                                      text1.text4.distance,
#                                      text2.text3.distance,
#                                      text2.text4.distance,
#                                      text3.text4.distance),
