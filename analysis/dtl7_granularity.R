...
window.df <- read_csv("../data/embeddings/lpm.windowed_umap.csv",
                      show_col_type = F) |> 
  group_by(story, window) |> 
  mutate(index = 1:n(),
         max = max(index),
...
ggsave(filename = here("../figures/lpm.first20.png"),
       units = "in",
       dpi = 450,
       width = 12, height = 7)
...
ggsave(filename = here("../figures/lpm.umap-window-unsmoothed.png"),
       units = "in",
       dpi = 450,
       width = 12, height = 7)
...
ggsave(paths.A,
       filename = here("../figures/lpm.umap-window-smoothed.png"),
       units = "in",
       dpi = 450,
       width = 12, height = 7)
...
iterative_dtw <- function(umap.embeds, window.x){
  ## ENG1, RUS2 ----
  eng1.rus2 <- dtw(umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 2" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  rus2.fre3 <- dtw(umap.embeds |> filter(story == "russian 2"  & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  rus2.fre3.distance <- rus2.fre3$normalizedDistance
  
  ## FRE3, ENG4 ----
  fre3.eng4 <- dtw(umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  fre3.eng4.distance <- fre3.eng4$normalizedDistance
  
  ## ENG4, ENG1 ----
  eng4.eng1 <- dtw(umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  eng4.eng1.distance <- eng4.eng1$normalizedDistance
  
  dtw.df <- data.frame(x.story = c("french 1","russian 2", "russian 3","french 4"),
                       y.story = c("russian 2", "russian 3","french 4","french 1"),
                       distance = c(eng1.rus2.distance,rus2.fre3.distance,
                                    fre3.eng4.distance, eng4.eng1.distance)) |> 
    mutate(window = window.x)
...
ggsave(filename = here("../figures/lpm.dtw-window-hm.png"),
       units = "in",
       dpi = 300,
       width = 17, height = 8)
...
ggsave(dtw.plot,
       filename = here("../figures/lpm.dtw-windowsize.png"),
       units = "in",
       dpi = 300,
       width = 12, height = 6)
...
full.dtw <- function(umap.embeds, window.x){
  ...
  ## ENG1, FRE3 ----
  eng1.fre3 <- dtw(umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  ## ENG1, ENG4 ----
  eng1.eng4 <- dtw(umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  ## FRE3, ENG1 ----
  fre3.eng1 <- dtw(umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  ## FRE3, RUS2 ----
  fre3.rus2 <- dtw(umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 2" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  ## ENG4, RUS2 ----
  eng4.rus2 <- dtw(umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 2" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  ## ENG4, FRE3 ----
  eng4.fre3 <- dtw(umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  eng1.eng1 <- dtw(umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "french 1" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  fre3.fre3 <- dtw(umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "russian 3" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  eng4.eng4 <- dtw(umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   umap.embeds |> filter(story == "french 4" & window == window.x) |> 
                     select(umap.x, umap.y),
                   keep = TRUE)
  ...
  dtw.df <- data.frame(story = c("french 1", "russian 2","russian 3", "french 4"),
                       `french 1` = c(eng1.eng1.distance, rus2.eng1.distance, fre3.eng1.distance, eng4.eng1.distance),
                       `russian 2` = c(eng1.rus2.distance, rus2.rus2.distance, fre3.rus2.distance, eng4.rus2.distance),
                       `russian 3` = c(eng1.fre3.distance, rus2.fre3.distance, fre3.fre3.distance, eng4.fre3.distance),
                       `french 4` = c(eng1.eng4.distance, rus2.eng4.distance, fre3.eng4.distance, eng4.eng4.distance)) |> 
    melt() |> 
    mutate(window = window)
...
ggsave(filename = here("../figures/lpm.dtw-window-full.png"),
       units = "in",
       dpi = 300,
       width = 18, height = 8)
...
ggsave(filename = here("../figures/lpm.dtw-window.png"),
       units = "in",
       dpi = 300,
       width = 12, height = 6)
